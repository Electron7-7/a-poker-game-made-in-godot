...
@onready var _template_save   : Button = $"%_SaveTemplate"
...
func Init() -> void:
    ...
    _new_button.pressed.connect(try_StartGame)
...
func toggle_saves() -> void:
    ...
    if(_saves.visible):
        for world in global_CasinoManager.GetWorldNames():
            var new_button : Button = _template_save.duplicate()
...
func try_StartGame() -> void:
    if(global_GameManager.StartNewGame() == OK):
...
func set_visible_items() -> void:
    ...
    if(!game_running):
        global_CasinoManager.CleanBadSaves()
        _query_saved_games()
...
func _query_saved_games() -> void:
    if(global_CasinoManager.HasSaveFiles()):
